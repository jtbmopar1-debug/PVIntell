...
const providerName: Record<string, string> = { junctek_local: "Junctek Bluetooth" };
...
export function MonitoringConnections() {
  ...
  return <main className="min-h-screen bg-canvas p-4 md:p-6"><div className="mx-auto max-w-3xl"><div className="flex items-center justify-between"><Link href="/settings" className="flex items-center gap-2 text-xs font-bold text-muted"><ArrowLeft size={15}/>Settings</Link><BrandLogo compact/></div><header className="mt-7"><div className="eyebrow">Monitoring setup</div><h1 className="mt-2 font-display text-2xl font-extrabold tracking-[-.04em]">Connect</h1><p className="mt-1.5 text-xs leading-5 text-muted">Connect and manage Junctek battery monitors. Other providers are temporarily hidden; their saved data is retained.</p></header>{error ? <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700">{error}</div> : null}{!data ? <div className="card mt-5 flex items-center gap-2 p-5 text-xs text-muted"><LoaderCircle size={15} className="animate-spin"/>Loading connections…</div> : <div className="mt-5 space-y-4">{data.systems.map((system) => { const rows = data.connections.filter((item) => item.project_id === system.id && item.provider === "junctek_local"); const site = data.sites.find((item) => item.id === system.site_id); return <section key={system.id} className="card overflow-hidden"><div className="border-b border-line bg-white px-4 py-3"><div className="text-sm font-extrabold">{system.name}</div><div className="mt-0.5 text-[10px] text-muted">{site?.name}</div><Link href={`/sites/${system.site_id}/systems/${system.id}?view=monitor`} className="mt-2 inline-block text-xs font-bold text-brand">Connect Junctek →</Link></div><div className="divide-y divide-line">{rows.length ? rows.map((connection) => <div key={connection.id} className="flex flex-wrap items-center gap-3 p-4"><span className={`grid size-9 place-items-center rounded-lg ${connection.is_active ? "bg-[#e2f3e9] text-[#207554]" : "bg-[#eef3f8] text-brand"}`}><Activity size={16}/></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><strong className="truncate text-xs">{connection.display_name}</strong>{connection.is_active ? <span className="rounded-md bg-[#e2f3e9] px-2 py-1 text-[9px] font-extrabold uppercase text-[#207554]">Active</span> : null}</div><div className="mt-1 text-[10px] text-muted">{providerName[connection.provider] ?? connection.provider}{connection.last_success_at ? ` · Last reading ${new Date(connection.last_success_at).toLocaleString()}` : ""}</div></div>{!connection.is_active ? <button disabled={busy === connection.id} onClick={() => void change(connection, "PATCH")} className="rounded-lg border border-line px-3 py-2 text-[10px] font-bold text-brand">Use this connection</button> : null}<button disabled={busy === connection.id} onClick={() => void change(connection, "DELETE")} aria-label={`Forget ${connection.display_name}`} className="grid size-8 place-items-center rounded-lg text-muted hover:bg-red-50 hover:text-red-700"><Trash2 size={14}/></button></div>) : <div className="p-4 text-xs text-muted">No monitoring connection has been assigned to this system.</div>}</div></section>; })}{!data.systems.length ? <div className="card p-6 text-xs text-muted">Create a system before assigning monitoring connections.</div> : null}<div className="flex items-start gap-3 rounded-xl border border-dashed border-line p-4 text-xs text-muted"><Link2 size={16} className="mt-0.5 shrink-0"/><span>New Bluetooth devices are assigned to the system whose Connect page started the connection. A device already assigned elsewhere is blocked until it is forgotten or reassigned.</span></div></div>}</div></main>;
}
