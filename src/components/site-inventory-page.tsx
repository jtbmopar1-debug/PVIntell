      ...
      <main className="min-w-0">
        <header className="sticky top-0 z-50 border-b border-line bg-[rgba(248,250,252,.98)]">
          <div className="mx-auto flex h-16 max-w-[1440px] items-center gap-4 px-4 md:px-6">
            ...
            <nav className="ml-auto hidden items-center gap-1 md:flex" aria-label="Primary navigation"><Link href={`/dashboard?site=${site.id}`} className="shrink-0 rounded-xl bg-[#f6c945] px-3 py-2 text-[11px] font-extrabold text-brand">Dashboard</Link><Link href={`/systems?site=${site.id}`} className="shrink-0 rounded-xl bg-[#f6c945] px-3 py-2 text-[11px] font-extrabold text-brand">Systems</Link><UniversalHowToMenu location={site.location} onAsk={askGuide}/><Link href={`/settings?site=${site.id}`} className="shrink-0 rounded-xl bg-[#f6c945] px-3 py-2 text-[11px] font-extrabold text-brand">Settings</Link></nav>
