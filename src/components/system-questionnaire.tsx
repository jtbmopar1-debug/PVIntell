...
export function SystemQuestionnaire({ answers, setAnswers, save, review, saving }: Props) {
  ...
  return <div className="animate-rise space-y-6">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      ...
      <div className="min-w-44 rounded-2xl border border-line bg-white p-4"><div className="flex justify-between text-xs"><strong>Required details</strong><span>{completed}/{required.length}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e6edf4]"><div className="h-full rounded-full bg-brand transition-all" style={{width:`${percent}%`}}/></div><div className="mt-2 flex items-center gap-1 text-[10px] text-muted"><Save size={11}/>{saving?"Saving…":"Saved as you go"}</div></div>
    </div>

    {systemDiscoveryQuestionnaire.sections.map((section, sectionIndex) => <section key={section.id} className="card overflow-hidden">
      <div className="border-b border-line bg-[#f7f9fc] px-6 py-5"><div className="eyebrow">Section {sectionIndex+1}</div><h2 className="mt-2 font-display text-xl font-extrabold">{section.title}</h2><p className="mt-1 text-xs text-muted">{section.description}</p></div>
