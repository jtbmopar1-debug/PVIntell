...
import { ArrowDownLeft, ArrowLeft, ArrowUpRight, BarChart3, Battery, ChevronDown, CloudSun, Fuel, Save, Sun } from "lucide-react";
...
export type MonitorSystem = { id: string; name: string; phase: string; mode: string; site: Site; arrays: SolarArrayForecastInput[] };
...
function SystemDailyLog({ system, onDirty }: { system: MonitorSystem; onDirty: (dirty: boolean) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-4"><div className="flex items-center justify-between gap-4"><h2 className="text-lg font-extrabold">History</h2><label className="text-xs font-bold">Period<select className="ml-2 rounded-lg border border-line bg-white p-2" value={range} onChange={(event) => setRange(Number(event.target.value))}>{[7, 30, 90, 365].map((value) => <option key={value} value={value}>Last {value} days</option>)}</select></label></div>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3"><Summary icon={Sun} label="Actual solar" value={summary.actualDays ? number(summary.actualKwh, " kWh") : "—"} detail={`${summary.actualDays} days reported`}/><Summary icon={CloudSun} label="Forecast comparison" value={summary.differencePercent === null ? "—" : `${summary.differencePercent > 0 ? "+" : ""}${number(summary.differencePercent, "%")}`} detail={`${summary.pairedDays} paired dates only`}/><Summary icon={Fuel} label="Generator runtime" value={summary.generatorRuntimeKnownDays ? number(summary.generatorMinutes / 60, " h") : "—"} detail={summary.generatorChargeEstimateDays ? `${number(summary.generatorChargeKwh, " kWh")} approx. charging energy` : `${summary.generatorDays} run days; ${summary.generatorRuntimeKnownDays} with duration`}/><Summary icon={ArrowDownLeft} label="Grid import" value={summary.gridImportDays ? number(summary.gridImportKwh, " kWh") : "—"} detail={`${summary.gridImportDays} days reported`}/><Summary icon={ArrowUpRight} label="Grid export" value={summary.gridExportDays ? number(summary.gridExportKwh, " kWh") : "—"} detail={`${summary.gridExportDays} days reported`}/><Summary icon={Battery} label="Days logged" value={String(summary.loggedDays)} detail="Missing days are not zeros"/></div>
...
export function DailyEntryForm({ systemId, date, today, existing, forecast, isOffGrid = false, weatherStatus, onSave, onDirty }: { systemId: string; date: string; today: string; existing?: DailyLogEntry; forecast?: ForecastSnapshot; isOffGrid?: boolean; weatherStatus: string; onSave: (entry: DailyLogEntry) => void; onDirty: (dirty: boolean) => void }) {
  ...
  const [gridExpanded, setGridExpanded] = useState(!isOffGrid || existing?.gridImportKwh != null || existing?.gridExportKwh != null);
  const approximateGeneratorChargeKwh = entry.generatorMinutes != null && entry.generatorChargingWatts != null
    ? entry.generatorChargingWatts * entry.generatorMinutes / 60 / 1000
    : null;
  const additionalInputKwh = (approximateGeneratorChargeKwh ?? 0) + (entry.gridImportKwh ?? 0);
  const hasAdditionalInput = approximateGeneratorChargeKwh != null || entry.gridImportKwh != null;
  ...
  const field = (key: "actualKwh" | "gridImportKwh" | "gridExportKwh" | "socMin" | "socMax" | "socEnd" | "generatorMinutes" | "generatorChargingWatts" | "generatorStartSoc" | "generatorEndSoc", label: string, unit: string) => <label className="text-xs font-bold">{label} <span className="font-normal text-muted">({unit})</span><input className="field" type="number" min={key === "generatorMinutes" || key === "generatorChargingWatts" ? .1 : 0} max={unit === "%" ? 100 : key === "generatorMinutes" ? 1440 : 1000000} step="any" value={entry[key] ?? ""} placeholder="Not recorded" onChange={(event) => set(key, event.target.value === "" ? null : Number(event.target.value))}/></label>;
  const generatorRuntimeField = <label className="text-xs font-bold">Total runtime <span className="font-normal text-muted">(hours)</span><input className="field" type="number" min="0.1" max="24" step="0.1" value={entry.generatorMinutes == null ? "" : entry.generatorMinutes / 60} placeholder="Not recorded" onChange={(event) => set("generatorMinutes", event.target.value === "" ? null : Number(event.target.value) * 60)}/></label>;
  ...
  return <form onSubmit={save} className="space-y-5 p-5"><fieldset disabled={busy} className="space-y-5 disabled:opacity-60">
    <div className="rounded-xl border border-[#e7cf78] bg-[#fff9df] p-4"><h3 className="text-sm font-extrabold">PVIntell solar forecast</h3><output aria-label="Forecast solar production" className="mt-2 block text-2xl font-extrabold text-[#6a5110]">{estimate ? number(estimate.kwh, " kWh") : "Not available"}</output>{hasAdditionalInput ? <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold">{approximateGeneratorChargeKwh != null ? <span className="rounded-lg border border-[#dfc66d] bg-white/55 px-2.5 py-1.5">Generator charging +{number(approximateGeneratorChargeKwh, " kWh")}</span> : null}{entry.gridImportKwh != null ? <span className="rounded-lg border border-[#dfc66d] bg-white/55 px-2.5 py-1.5">Grid import +{number(entry.gridImportKwh, " kWh")}</span> : null}{estimate ? <output aria-label="Forecast plus additional energy" className="rounded-lg bg-[#6a5110] px-2.5 py-1.5 text-white">Simple input total {number(estimate.kwh + additionalInputKwh, " kWh")}</output> : null}</div> : null}<p className="mt-2 text-xs leading-5 text-muted">{existing?.forecast ? `Saved forecast · captured ${new Date(existing.forecast.capturedAt).toLocaleString()}. This value is fixed; editing your readings leaves it unchanged.` : estimate ? "Automatically loaded. This forecast is saved with your daily entry and cannot be edited." : weatherStatus}{hasAdditionalInput ? " The simple input total keeps solar, generator and grid labelled separately; it is not measured consumption, SOC change or a complete energy balance." : ""}</p></div>
    {date <= today ? <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{field("actualKwh", "Actual solar production", "kWh")}{field("socMin", "Lowest battery SOC", "%")}{field("socMax", "Highest battery SOC", "%")}{field("socEnd", "End-of-day SOC", "%")}</div><p className="text-[11px] text-muted">Enter the whole-day PV total, excluding generator and grid energy. Leave readings you don’t have blank.</p>
      <div className="rounded-xl border border-line p-4"><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" className="size-4 accent-[#195e98]" checked={entry.generatorRan === true} onChange={(event) => { setEntry((previous) => ({ ...previous, generatorRan: event.target.checked ? true : null, ...(event.target.checked ? {} : { generatorMinutes: null, generatorChargingWatts: null, generatorStartSoc: null, generatorEndSoc: null }) })); onDirty(true); }}/>Ran generator this day</label>
        {entry.generatorRan ? <div className="mt-4"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{generatorRuntimeField}{field("generatorChargingWatts", "Approx. charging power", "W")}{field("generatorStartSoc", "SOC before first run", "%")}{field("generatorEndSoc", "SOC after last run", "%")}</div>{approximateGeneratorChargeKwh != null ? <output className="mt-3 block rounded-lg bg-[#eef5fc] px-3 py-2 text-[11px]" aria-label="Approximate generator charging energy"><strong>Approx. generator charging energy: {number(approximateGeneratorChargeKwh, " kWh")}</strong><span className="ml-1 text-muted">calculated from runtime × charging power. This is energy, not an SOC percentage.</span></output> : null}<p className="mt-3 text-[11px] leading-5 text-muted">Runtime never includes grid input. Enter approximate battery-side charging power if known—not the generator’s rated output. If it ran more than once, add the durations together and describe changing loads or charge rates below.</p></div> : null}
      </div>
      <div className="rounded-xl border border-line"><button type="button" onClick={() => setGridExpanded((expanded) => !expanded)} aria-expanded={gridExpanded} className="flex w-full items-center justify-between gap-3 p-4 text-left"><span><strong className="text-sm">Grid use</strong> <span className="text-xs text-muted">(optional{isOffGrid ? " · off-grid" : ""})</span></span><span className="flex items-center gap-2 text-[10px] font-bold text-brand">{gridExpanded ? "Minimise" : "Add grid readings"}<ChevronDown size={15} className={`transition-transform ${gridExpanded ? "rotate-180" : ""}`}/></span></button>{gridExpanded ? <div className="border-t border-line p-4"><div className="grid gap-4 sm:grid-cols-2">{field("gridImportKwh", "Grid import / input", "kWh")}{field("gridExportKwh", "Grid export", "kWh")}</div><p className="mt-3 text-[11px] leading-5 text-muted">Import is energy taken from the public grid; export is energy sent back. Enter each day’s total, not the meter’s lifetime reading. Leave blank if off-grid or unknown; enter 0 only for a known zero. Keep solar production separate—don’t add export to it.</p></div> : null}</div>
