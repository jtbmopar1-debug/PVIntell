...
export function SystemMonitor({ project, site }: { project: Project; site: Site }) {
  ...
  return <div className="animate-rise space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="eyebrow">Connect</div><h1 className="mt-2 font-display text-3xl font-extrabold">{project.name}</h1><p className="mt-2 text-xs text-muted">Junctek connection and readings for this system at {site.name}.</p></div><button type="button" onClick={() => void refresh()} disabled={busy} className="flex items-center gap-2 rounded-xl border border-line bg-white px-3 py-2 text-xs font-bold text-brand"><RefreshCw size={14} className={busy ? "animate-spin" : ""}/>Refresh</button></div>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Tile label="Status" text={statusNames[state]} icon={Activity}/><Tile label="Provider" text={connected || data?.connection ? "Junctek" : "Not connected"} icon={CircleGauge}/><Tile label="Last reading" text={latest ? new Date(latest.measuredAt).toLocaleString() : "Not reported"} icon={RefreshCw}/><Tile label="Active alerts" text={String(data?.alerts.length ?? 0)} icon={AlertTriangle}/></div>
    ...
    {latest ? <JunctekReadingPanel latest={latest} deviceName={localHere ? local.deviceName : data?.devices[0]?.displayName}/> : <Notice icon={CircleGauge} title="No readings yet" detail="A saved connection does not imply a live Bluetooth session. Connect the nearby monitor to receive readings."/>}
    {data?.samples.length ? <section className="card p-4"><div className="eyebrow">Recent Junctek readings</div><div className="mt-3 max-h-60 overflow-auto">{data.samples.slice(-24).reverse().map((sample) => <div key={sample.measuredAt} className="grid grid-cols-4 gap-2 border-b border-line py-2 text-[10px]"><span>{new Date(sample.measuredAt).toLocaleString()}</span><span>{preciseReading(sample.batteryVoltageV, "V")}</span><span>{preciseReading(sample.batteryCurrentA, "A")}</span><span>{preciseReading(sample.batterySocPercent, "% SOC")}</span></div>)}</div></section> : null}
  </div>;
}
