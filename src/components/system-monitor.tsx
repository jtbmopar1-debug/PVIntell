...
export function SystemMonitor({ project, site }: { project: Project; site: Site }) {
  ...
  const [localStatus, setLocalStatus] = useState(""); const [localConnected, setLocalConnected] = useState(false); const localSession = useRef<LocalDeviceSession | null>(null); const lastLocalUpload = useRef(0);
  ...
  async function connectJunctek() {
    ...
    try {
      ...
      localSession.current = session; setLocalConnected(true); setLocalStatus(`Connected to ${session.displayName}; waiting for its first reading…`);
  ...
  function disconnectJunctek() {
    localSession.current?.disconnect(); localSession.current = null; lastLocalUpload.current = 0;
    setLocalConnected(false); setLocalStatus(""); setError("");
  }
  async function disconnectMonitoring() {
    const connection = data?.connection; disconnectJunctek();
