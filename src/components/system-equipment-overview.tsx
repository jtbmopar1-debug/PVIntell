...
export function SystemEquipmentOverview({ project, onAskWattson }: { project: Project; onAskWattson: () => void }) {
  ...
  return <div className="animate-rise space-y-7">
    ...
    <section>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="eyebrow">As-built system</div><h2 className="mt-2 text-xl font-extrabold">Confirmed installed equipment</h2><p className="mt-1 text-xs text-muted">Only confirmed equipment is shown. Drag cards to arrange the record.</p>{orderMessage && <p className="mt-2 text-[10px] font-semibold text-brand">{orderMessage}</p>}</div><div className="flex gap-2"><button onClick={() => router.push(`${base}/equipment/new?type=panel&name=PV%20array`)} className="flex items-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-xs font-bold text-white"><Plus size={14}/>Add PV array</button><button onClick={() => router.push(`${base}/equipment/new`)} className="flex items-center gap-2 rounded-xl border border-line bg-white px-4 py-2.5 text-xs font-bold"><Plus size={14}/>Add other</button></div></div>

      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {arrays.map((array) => { const watts = (array.panelWatts ?? 0) * (array.panelCount ?? 0); return <button key={array.id} {...dragProps(`pv:${array.id}`)} onClick={() => router.push(`${base}/equipment/${array.id}`)} className="card cursor-grab p-5 text-left hover:border-[#7aa6d1] active:cursor-grabbing"><div className="flex justify-between"><span className="grid size-10 place-items-center rounded-xl bg-[#eaf2fb] text-brand"><Sun size={19}/></span><span className="rounded-full bg-[#e7f0fb] px-2 py-1 text-[9px] font-bold uppercase text-[#175a96]">Confirmed</span></div><h3 className="mt-4 text-sm font-bold">{array.name}</h3><p className="mt-1 text-[10px] text-muted">{array.panelCount ?? "?"} × {array.panelWatts ?? "?"} W panels{watts ? ` · ${(watts / 1000).toFixed(2)} kW` : ""}</p><div className="mt-4 grid grid-cols-2 gap-2 text-[10px]"><Mini label="PV connection" value={String(array.specifications?.["PV connection"] ?? "") || undefined}/><Mini label="Panel type" value={array.panelType}/><Mini label="Cable" value={array.cableSizeMm2 ? `${array.cableSizeMm2} mm²` : undefined}/><Mini label="Length" value={array.cableLengthM ? `${array.cableLengthM} m` : undefined}/><Mini label="Connectors" value={array.connectorType}/></div><span className="mt-4 block text-[10px] font-bold text-brand">Open panel record →</span></button>; })}
