...
export function SystemEquipmentOverview({ project, onAskWattson }: { project: Project; onAskWattson: () => void }) {
  ...
  return <div className="animate-rise space-y-7">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div><div className="eyebrow">As-built record</div><h1 className="mt-3 font-display text-3xl font-extrabold tracking-[-.05em] md:text-[38px]">{project.name} as-built overview</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Record only equipment that actually exists. Proposed equipment belongs in Proposed design until it is purchased and installed.</p></div>
      <div className="flex flex-wrap gap-2"><button onClick={() => router.push(`${base}/design`)} className="flex h-11 items-center gap-2 rounded-xl border border-line bg-white px-4 text-xs font-bold text-brand"><Zap size={15}/>Proposed design</button><button onClick={() => router.push(`${base}/schematic`)} className="flex h-11 items-center gap-2 rounded-xl border border-line bg-white px-4 text-xs font-bold text-brand"><Waypoints size={15}/>As-built schematic</button><button onClick={() => router.push(`${base}/photos`)} className="flex h-11 items-center gap-2 rounded-xl border border-line bg-white px-4 text-xs font-bold"><Camera size={15}/>Photos</button><button onClick={onAskWattson} className="flex h-11 items-center gap-2 rounded-xl bg-brand px-5 text-xs font-bold text-white"><Zap size={15}/>Tell Wattson about equipment</button></div>
    </div>

    {project.designCalculator?.proposedAsBuiltDraft && <section className="rounded-2xl border border-[#8ab0d2] bg-[#f2f8fe] p-4 text-xs leading-5 text-[#143c63]"><strong>Reviewed proposed schematic saved as a draft.</strong> It can guide this future as-built record, but it has not added or confirmed any equipment or connections. Open Proposed design whenever you need to compare the planning layout with what is actually installed.</section>}
