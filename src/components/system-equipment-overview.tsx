...
export function SystemEquipmentOverview({ project, onAskWattson }: { project: Project; onAskWattson: () => void }) {
  ...
  return <div className="animate-rise space-y-7">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div><div className="eyebrow">As-built record</div><h1 className="mt-3 font-display text-3xl font-extrabold tracking-[-.05em] md:text-[38px]">{project.name} as-built equipment</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Record only equipment that actually exists in this system.</p></div>
    ...
    <section>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        {!arrays.length && !components.length ? <div className="card p-5 text-xs text-muted">No confirmed installed equipment records yet.</div> : null}
      </div>
    </section>
  </div>;
}
