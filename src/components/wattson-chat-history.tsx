...
export function WattsonChatHistory({ initialChats, limit }: { initialChats: WattsonChatSummary[]; limit: number }) {
  ...
  }, [selected?.id, selected?.messages.length, sending]);
  ...
  return <>
    ...
    <div className="mt-4 grid gap-3">{chats.map((chat) => <article key={`${chat.kind}:${chat.id}`} className="card flex min-w-0 items-start gap-3 p-4"><span className={`mt-0.5 grid size-9 shrink-0 place-items-center rounded-lg ${chat.kind === "system" ? "bg-[#fff1bc] text-[#a36a00]" : "bg-[#eaf2fb] text-brand"}`}><MessageSquareText size={17}/></span><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><button type="button" onClick={() => setSelected(chat)} className="block max-w-full truncate text-left text-sm font-extrabold hover:text-brand">{chat.title}</button><p className="mt-0.5 text-[11px] font-semibold text-muted">{chat.context} · {chat.messageCount} messages</p></div><time className="text-[10px] text-muted">{new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(new Date(chat.updatedAt))}</time></div><p className="mt-2 line-clamp-2 text-[12px] leading-5 text-muted">{chat.preview || "No messages in this conversation yet."}</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => setSelected(chat)} className="inline-flex h-9 items-center gap-2 rounded-lg bg-brand px-3 text-[11px] font-bold text-white">Open chat <MessageSquareText size={13}/></button><button type="button" onClick={() => void remove(chat)} disabled={deleting === chat.id} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[#e5c1b8] px-3 text-[11px] font-bold text-[#9a4937] disabled:opacity-50"><Trash2 size={13}/>{deleting === chat.id ? "Deleting…" : "Delete"}</button></div></div></article>)}
      {!chats.length ? <div className="card p-8 text-center"><Bot className="mx-auto text-brand"/><h2 className="mt-3 text-base font-extrabold">No saved Wattson chats yet</h2><p className="mt-1 text-xs text-muted">Conversations will appear here after you chat from the dashboard or a system.</p><Link href="/wattson" className="mt-4 inline-flex h-10 items-center rounded-lg bg-brand px-4 text-xs font-bold text-white">Ask Wattson</Link></div> : null}
    </div>
    {selected ? <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#0b2740]/50 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label={selected.title} onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(undefined); }}><section className="flex h-[calc(100dvh-0.75rem)] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl border border-line bg-white shadow-2xl sm:h-[min(86dvh,760px)] sm:rounded-2xl"><header className="flex shrink-0 items-center gap-3 border-b border-line bg-[linear-gradient(100deg,#eaf3fb,#fff6ce)] p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand text-white"><Bot size={19}/></span><div className="min-w-0 flex-1"><div className="eyebrow">Wattson · {selected.context}</div><h2 className="mt-1 truncate text-base font-extrabold">{selected.title}</h2></div><button type="button" onClick={() => setSelected(undefined)} className="grid size-10 place-items-center rounded-xl border border-line bg-white text-muted" aria-label="Close conversation"><X size={18}/></button></header><div ref={chatScrollRef} className="thin-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto bg-white p-3 sm:p-5" aria-live="polite">{selected.messages.map((message, index) => <div key={`${message.createdAt}:${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[92%] rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[82%] ${message.role === "user" ? "bg-brand text-white" : "wattson-assistant-message bg-[#edf2f7] text-ink"}`}><FormattedChatMessage content={message.content}/><time className={`mt-2 block text-[11px] leading-4 ${message.role === "user" ? "text-white/70" : "text-muted"}`}>{new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(new Date(message.createdAt))}</time></div></div>)}{sending ? <p className="text-sm text-muted">Wattson is thinking…</p> : null}</div><form onSubmit={(event) => { event.preventDefault(); void send(); }} className="shrink-0 border-t border-line bg-white p-3 sm:p-4"><div className="flex items-end gap-2"><textarea value={input} onChange={(event) => setInput(event.target.value)} rows={3} placeholder="Ask Wattson…" className="field wattson-composer-input mt-0 min-h-20 flex-1 resize-none py-3 text-base leading-6"/><button disabled={!input.trim() || sending} className="grid size-12 shrink-0 place-items-center rounded-xl bg-brand text-white disabled:opacity-40" aria-label="Send to Wattson"><Send size={18}/></button></div></form></section></div> : null}
  </>;
}
