...
export function AccountLocationPlanner({ sites }: { sites: PlanningSite[] }) {
  ...
  const [newSiteLocationSource, setNewSiteLocationSource] = useState<"search" | "device">("search");
  const [searching, setSearching] = useState(false);
  const committedLocationQuery = useRef(firstSite?.location && firstSite.location !== "Location not set" ? firstSite.location : "");
  ...
  function selectSite(id: string) { const next = sites.find((item) => item.id === id); const nextQuery = next?.location && next.location !== "Location not set" ? next.location : ""; setSiteId(id); setActivePointId(next?.systems[0] ? systemKey(next.systems[0].id) : ""); committedLocationQuery.current = nextQuery; setLocationQuery(nextQuery); setLocationMatches([]); setNewSitePosition(undefined); setNewSiteLocationSource("search"); setNotice(""); }
  ...
  function chooseSiteLocation(match: LocationMatch, source: "search" | "device" = "search") { committedLocationQuery.current = match.label; setNewSitePosition(match); setNewSiteLocationSource(source); setLocationQuery(match.label); setLocationMatches([]); setNotice(""); }
  function useDeviceForSite() {
    if (!navigator.geolocation) return setNotice("Location access is not available on this device.");
    navigator.geolocation.getCurrentPosition(({ coords }) => chooseSiteLocation({ name: site?.name ?? "Site", label: site?.name ?? "Device location", latitude: coords.latitude, longitude: coords.longitude, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC" }, "device"), () => setNotice("PVIntell could not access this device’s location."), { enableHighAccuracy: true, timeout: 12000 });
  }
  async function saveSiteLocation() {
    if (!site || !newSitePosition) return;
    setSaving(true); setNotice("");
    try {
      const response = await fetch(`/api/sites/${site.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ location: newSitePosition.label, latitude: newSitePosition.latitude, longitude: newSitePosition.longitude, timezone: newSitePosition.timezone, locationSource: newSiteLocationSource }) });
  ...
  async function createSiteLocation() {
    if (!newSiteName.trim() || !newSitePosition) return setNotice("Enter a Site name and choose its location.");
    setSaving(true); setNotice("");
    try {
      const response = await fetch("/api/sites", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: newSiteName, location: newSitePosition.label, latitude: newSitePosition.latitude, longitude: newSitePosition.longitude, timezone: newSitePosition.timezone, locationSource: newSiteLocationSource }) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error || "Could not create the Site.");
      router.refresh();
    } catch (problem) { setNotice(problem instanceof Error ? problem.message : "Could not create the Site."); } finally { setSaving(false); }
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const body = await response.json(); if (!response.ok) throw new Error(body.error || "Could not search locations.");
        setLocationMatches(body.results ?? []);
      } catch (problem) { if (!controller.signal.aborted) setNotice(problem instanceof Error ? problem.message : "Could not search locations."); }
  ...
  return <section className="card mt-4 overflow-hidden">
    ...
    {site && sitePosition && guidance ? <>
      ...
      <div className="mx-auto max-w-xl text-center"><MapPin className="mx-auto text-brand"/><h3 className="mt-3 text-sm font-extrabold">{site ? `${site.name} is saved, but not mapped yet` : "Add a Site location"}</h3><p className="mt-1 text-xs leading-5 text-muted">Search for the Site, use the device location, then refine the pin if needed. This is available before or after Discovery.</p></div>
      <div className="mx-auto mt-5 max-w-xl">
        {!site ? <label className="block text-xs font-bold">Site name<input value={newSiteName} onChange={(event) => setNewSiteName(event.target.value)} className="field" placeholder="e.g. Home, Farm or Workshop"/></label> : null}
        <div className="flex gap-2"><input value={locationQuery} onChange={(event) => { setLocationQuery(event.target.value); if (event.target.value.trim().length < 2) setLocationMatches([]); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void searchSiteLocation(); } }} className="field mt-0" placeholder="Address, town, postcode or region" autoComplete="off"/><button type="button" onClick={() => void searchSiteLocation()} disabled={searching || locationQuery.trim().length < 2} className="grid size-11 shrink-0 place-items-center rounded-xl bg-brand text-white disabled:opacity-40">{searching ? <LoaderCircle className="animate-spin" size={17}/> : <Search size={17}/>}</button></div>
        <button type="button" onClick={useDeviceForSite} className="mt-2 inline-flex items-center gap-2 rounded-xl border border-line bg-white px-3 py-2 text-[11px] font-bold"><LocateFixed size={14}/>Enable and use device location</button>
        {locationMatches.length ? <div className="mt-3 overflow-hidden rounded-xl border border-line">{locationMatches.map((match) => <button key={`${match.latitude}:${match.longitude}`} type="button" onClick={() => chooseSiteLocation(match)} className="block w-full border-b border-line bg-white px-4 py-3 text-left text-xs last:border-0 hover:bg-[#edf5fd]"><strong>{match.name}</strong><span className="mt-1 block text-[11px] text-muted">{match.label}</span></button>)}</div> : null}
        {newSitePosition ? <><div className="relative mt-4 h-64 overflow-hidden rounded-xl border border-line bg-[#dfe9ee]"><EditableSiteMap points={[{ id: site ? siteKey(site.id) : "new-site", name: site?.name ?? (newSiteName || "New Site"), latitude: newSitePosition.latitude, longitude: newSitePosition.longitude, kind: "site" }]} activeId={site ? siteKey(site.id) : "new-site"} center={[newSitePosition.latitude, newSitePosition.longitude]} onSelect={() => undefined} onMove={(_, latitude, longitude) => setNewSitePosition((current) => current ? { ...current, latitude, longitude } : current)}/></div><label className="mt-3 block text-xs font-bold">Saved address or location<input value={newSitePosition.label} onChange={(event) => setNewSitePosition((current) => current ? { ...current, label: event.target.value } : current)} className="field"/></label><button type="button" onClick={() => void (site ? saveSiteLocation() : createSiteLocation())} disabled={saving || (!site && !newSiteName.trim())} className="mt-3 flex h-10 items-center gap-2 rounded-xl bg-brand px-4 text-xs font-bold text-white disabled:opacity-40"><Save size={14}/>{saving ? "Saving…" : site ? `Save ${site.name} location` : "Create Site"}</button></> : null}
