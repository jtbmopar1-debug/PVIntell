...
export function SiteEquipmentInventory({ siteId, projectId, initialEquipment, initialEditId }: { siteId: string; projectId: string; initialEquipment: SiteEquipment[]; initialEditId?: string }) {
  ...
  return <div className="animate-rise space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="eyebrow">Site inventory</div><h1 className="mt-3 font-display text-3xl font-extrabold tracking-[-.05em] md:text-[38px]">Equipment you already have</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Record new, second-hand or inherited equipment here before deciding which system should use it.</p></div><button onClick={() => { setEditingItem(undefined); setExtraction(undefined); setPhotoPath(""); setAdding(true); }} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-brand px-5 text-xs font-bold text-white"><Plus size={15}/>Add equipment</button></div>
    ...
    {adding && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#0b2740]/45 p-5 backdrop-blur-sm"><form key={editingItem?.id ?? "new"} ref={formRef} action={submit} className="card my-8 w-full max-w-2xl bg-white p-6 shadow-2xl"><div className="flex justify-between"><div><div className="eyebrow">Site inventory</div><h2 className="mt-2 font-display text-2xl font-extrabold">{editingItem ? "Edit equipment" : "Add equipment"}</h2></div><button type="button" onClick={() => { setAdding(false); setEditingItem(undefined); }} className="grid size-8 place-items-center rounded-lg hover:bg-[#eef3f8]"><X size={17}/></button></div>
      <label className="mt-5 flex cursor-pointer items-center gap-4 rounded-2xl border border-dashed border-[#91aec9] bg-[#f7fafd] p-4"><span className="grid size-11 place-items-center rounded-xl bg-white text-brand"><Camera size={20}/></span><span className="flex-1"><strong className="block text-xs">Scan an equipment label</strong><span className="mt-1 block text-[10px] text-muted">JPEG, PNG or WebP, up to 8 MB. Review every field before saving.</span></span>{analyzing ? <LoaderCircle className="animate-spin text-brand" size={19}/> : <ScanLine className="text-brand" size={19}/>}<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="hidden" disabled={analyzing} onChange={(event) => void analyze(event.target.files?.[0])}/></label>
