...
export function SiteEquipmentInventory({ siteId, projectId, initialEquipment, initialEditId }: { siteId: string; projectId: string; initialEquipment: SiteEquipment[]; initialEditId?: string }) {
  ...
  return <div className="animate-rise space-y-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><div className="eyebrow">Site inventory</div><h1 className="mt-2 font-display text-2xl font-extrabold tracking-[-.045em] md:text-[30px]">Equipment you already have</h1><p className="mt-1.5 max-w-2xl text-xs leading-5 text-muted">Record new, second-hand or inherited equipment here before deciding which system should use it.</p></div><button onClick={() => { setEditingItem(undefined); setExtraction(undefined); setPhotoPath(""); setAdding(true); }} className="flex h-9 items-center justify-center gap-2 rounded-lg bg-brand px-4 text-[10px] font-bold text-white"><Plus size={14}/>Add equipment</button></div>
