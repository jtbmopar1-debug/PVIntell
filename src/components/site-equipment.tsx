...
export function SiteEquipmentInventory({ siteId, projectId, initialEquipment, initialEditId }: { siteId: string; projectId: string; initialEquipment: SiteEquipment[]; initialEditId?: string }) {
  ...
  return <div className="animate-rise space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="eyebrow">Site inventory</div><h1 className="mt-3 font-display text-3xl font-extrabold tracking-[-.05em] md:text-[38px]">Equipment you already have</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Record new, second-hand or inherited equipment here before deciding which system should use it.</p></div><button onClick={() => { setEditingItem(undefined); setExtraction(undefined); setPhotoPath(""); setAdding(true); }} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-brand px-5 text-xs font-bold text-white"><Plus size={15}/>Add equipment</button></div>
    <div className="flex gap-3 rounded-2xl border border-[#ecd9aa] bg-[#fff8e7] p-4 text-[#6f5518]"><AlertTriangle size={18} className="shrink-0"/><p className="text-[11px] leading-5"><strong>Second-hand equipment needs verification.</strong> Wattson can compare recorded ratings, but condition, battery health, firmware, protection requirements and regional compliance may still require testing or professional inspection.</p></div>
    {items.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={item.id} className="card p-5"><div className="flex justify-between"><span className="grid size-9 place-items-center rounded-xl bg-[#edf3e9] text-brand"><PackagePlus size={17}/></span><span className="rounded-full bg-[#fff1cf] px-2 py-1 text-[9px] font-bold uppercase text-[#8b6512]">{item.status}</span></div><h2 className="mt-4 text-sm font-bold">{item.name}</h2><p className="mt-1 text-[10px] text-muted">{[item.manufacturer, item.model].filter(Boolean).join(" • ") || typeLabels[item.type]}</p>{item.serialNumber && <p className="mt-1 text-[9px] text-muted">Serial: {item.serialNumber}</p>}<div className="mt-4 grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-xl bg-[#f1f4ef] p-2"><span className="block text-muted">Quantity</span><strong>{item.quantity}</strong></div><div className="rounded-xl bg-[#f1f4ef] p-2"><span className="block text-muted">Condition</span><strong>{conditionLabels[item.condition]}</strong></div></div><button onClick={() => { setEditingItem(item); setExtraction(undefined); setPhotoPath(""); setAdding(true); }} className="mt-4 w-full rounded-xl border border-line py-2 text-[10px] font-bold text-brand">Edit details or rescan label</button></article>)}</div> : <div className="card grid min-h-52 place-items-center p-8 text-center"><div><PackagePlus className="mx-auto text-muted"/><h2 className="mt-4 text-sm font-bold">No equipment recorded yet</h2><p className="mt-2 text-xs text-muted">Add anything you already own or may want Wattson to consider.</p></div></div>}
    {adding && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#10251c]/45 p-5 backdrop-blur-sm"><form key={editingItem?.id ?? "new"} ref={formRef} action={submit} className="card my-8 w-full max-w-2xl bg-white p-6 shadow-2xl"><div className="flex justify-between"><div><div className="eyebrow">Site inventory</div><h2 className="mt-2 font-display text-2xl font-extrabold">{editingItem ? "Edit equipment" : "Add equipment"}</h2></div><button type="button" onClick={() => { setAdding(false); setEditingItem(undefined); }} className="grid size-8 place-items-center rounded-lg hover:bg-[#eef2ec]"><X size={17}/></button></div>
      <label className="mt-5 flex cursor-pointer items-center gap-4 rounded-2xl border border-dashed border-[#9db1a2] bg-[#f5f8f2] p-4"><span className="grid size-11 place-items-center rounded-xl bg-white text-brand"><Camera size={20}/></span><span className="flex-1"><strong className="block text-xs">Scan an equipment label</strong><span className="mt-1 block text-[10px] text-muted">JPEG, PNG or WebP, up to 8 MB. Review every field before saving.</span></span>{analyzing ? <LoaderCircle className="animate-spin text-brand" size={19}/> : <ScanLine className="text-brand" size={19}/>}<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="hidden" disabled={analyzing} onChange={(event) => void analyze(event.target.files?.[0])}/></label>
      {error && <div className="mt-4 rounded-xl bg-[#fff0eb] p-3 text-xs text-[#9c3e2e]">{error}</div>}{extraction && <div className="mt-4 rounded-xl border border-line bg-[#f7f9f5] p-3 text-[10px]"><div className="flex items-center justify-between"><strong>Label details added for review</strong><span className="uppercase text-brand">{extraction.confidence} confidence</span></div>{extraction.warnings.length > 0 && <p className="mt-2 text-[#8b6512]">{extraction.warnings.join(" ")}</p>}{extraction.unreadableFields.length > 0 && <p className="mt-1 text-muted">Could not read: {extraction.unreadableFields.join(", ")}</p>}</div>}
