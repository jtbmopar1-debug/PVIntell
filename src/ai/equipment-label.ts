...
export async function extractEquipmentLabel(image: Uint8Array, mimeType: string): Promise<EquipmentLabelExtraction> {
  ...
  const response = await fetch("https://generativelanguage.googleapis.com/v1beta/interactions", {
    ...
    body: JSON.stringify({
      model,
      store: false,
      system_instruction: "Read solar and electrical equipment nameplates conservatively. Extract only text that is visibly supported by the image. For PV panels, explicitly look for Pmax, Vmp, Imp, Voc, Isc, maximum system voltage, NOCT, maximum series fuse rating, power tolerance, module efficiency, temperature coefficients, dimensions, weight, and whether the label states bifacial or another panel construction. For batteries, preserve any printed chemistry (such as flooded lead-acid, AGM, gel, LFP/LiFePO4, other lithium-ion, or sodium-ion), nominal and charge voltage, Ah and kWh capacity, continuous/peak current, BMS details, manufacture date and temperature limits in the standard fields or otherSpecifications. Do not infer battery chemistry from case shape, colour or marketing appearance. Use empty strings for absent fields. Preserve printed units. Never infer compatibility, condition, safety, approval, or regulatory compliance from a label. Put ambiguity, possible OCR errors, or multiple possible values in warnings.",
      input: [
