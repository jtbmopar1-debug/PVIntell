...
export function discoveryGuidance(key: string) {
  const guidance: Record<string, string> = {
    ...
    ac_phase_arrangement: "Most ordinary houses use single-phase power, commonly about 230 V in New Zealand and many countries or 110–120 V in some overseas systems. Confirm from a meter, supply document, main-switch label or existing inverter label. Never infer the phase count from the number of switch or breaker toggles: older single-phase boards can have linked multi-pole devices. Do not open an electrical enclosure to investigate it.",
    ...
    generator_details: "If a generator is included but not purchased yet, choose that option and continue: the proposal should specify a continuous and surge target to shop for. If one already exists or has been chosen, use its label for continuous/surge power, voltage, phase, fuel, start method and connection details.",
    generator_outage_role: "Tell me whether the generator should recharge batteries, carry selected high-power loads such as a welder, supply the backed-up circuits, start automatically at low reserve, or perform a compatible combination of those roles.",
    ...
    hot_water_storage_litres: "Tell me the separate indoor HWC, cylinder or geyser volume in litres from its label if possible. Do not duplicate a shared solar/backup tank.",
    solar_hot_water_arrangement: "Tell me whether the solar hot-water system is pumped or thermosiphon, and whether it uses one shared boosted tank or a solar preheat tank feeding a separate indoor HWC.",
    solar_hot_water_storage_litres: "Tell me the solar hot-water store volume in litres. Keep it separate from any second indoor HWC.",
    solar_hot_water_pump_watts: "Use the solar circulation-pump label to record its real electrical input in watts.",
    solar_hot_water_pump_hours_per_day: "Use controller history or monitoring if possible to estimate how many hours the circulation pump operates on a useful solar day.",
...
export function nextRequiredDiscoveryQuestion(
  ...
  const steps = [
    ["ac_phase_arrangement", "What AC phase arrangement is available or required at this Site: single-phase, split-phase, three-phase, DC-only, or not yet known? Most houses are single-phase, but confirm it from reliable records or labelling—not from the number of switch toggles."],
    ...
    ...(/include|existing|planned|provision[_ ]only/i.test(valueOf("generator_requirement")) ? [["generator_details", "Has the generator already been purchased or selected? If not, record that and continue so the proposal can specify the continuous and surge target. Otherwise record its make/model, fuel, output, voltage/phase, start method and connection."]] : []),
    ...(isShedOrWorkshop ? [
      ...
      ...(/electric|heat.?pump|solar|wetback/i.test(valueOf("water_heating_energy")) ? [["hot_water_storage_litres", "What is the hot-water tank or cylinder storage volume in litres? Use the label if possible; include a solar hot-water storage tank too."]] : []),
