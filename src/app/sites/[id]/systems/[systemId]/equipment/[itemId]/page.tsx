...
export default async function EquipmentPage({ params, searchParams }: { params: Promise<{ id: string; systemId: string; itemId: string }>; searchParams: Promise<{ type?: string; name?: string }> }) {
  ...
  return <ComponentDetail siteId={id} systemId={systemId} systemName={workspace.project.name} component={component} defaults={component ? undefined : { kind, name: query.name?.slice(0,120) || "New equipment" }}/>;
}
