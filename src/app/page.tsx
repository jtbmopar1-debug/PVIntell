...
export default async function Home({searchParams}:{searchParams:Promise<{welcome?:string}>}) {
  ...
  return <PVIntellWorkspace initialProject={project} initialMessages={messages.length?messages:initialConversation} cloud email={typeof data?.claims?.email==="string"?data.claims.email:""} showGoogleWelcome={query.welcome==="google"}/>;
}
