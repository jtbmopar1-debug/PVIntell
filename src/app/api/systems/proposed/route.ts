...
const componentSchema = z.object({ id: z.uuid().optional(), type: z.enum(["inverter", "battery", "generator"]), name: z.string().trim().min(1).max(120), manufacturer: z.string().trim().max(120).optional(), model: z.string().trim().max(120).optional(), quantity: z.number().int().positive().max(100), rating: optionalNumber, batteryKwh: optionalNumber, batteryAh: optionalNumber, capacityInputBasis: z.enum(["kWh", "Ah"]).optional(), batteryType: z.string().trim().max(80).optional(), bmsCompatibility: z.string().trim().max(80).optional(), voltage: optionalNumber, batteryVoltageMin: optionalNumber, batteryVoltageMax: optionalNumber, inverterType: z.enum(["hybrid", "grid_tied_string", "off_grid_inverter_charger", "battery_inverter", "microinverter", "other"]).optional(), generatorFuelType: z.enum(["petrol", "diesel", "lpg", "natural_gas", "dual_fuel", "other"]).optional(), mpptInputs: z.number().int().positive().max(100).optional(), mpptMin: optionalNumber, mpptMax: optionalNumber, maxPvVoltage: optionalNumber, maxInputCurrent: optionalNumber, notes: z.string().trim().max(2000).optional() });
...
function compatibilityIssues(input: z.infer<typeof schema>) {
  ...
  for (const inverter of inverters) {
    const missing = [!inverter.manufacturer && "manufacturer", !inverter.model && "exact model", !inverter.inverterType && "inverter type", !inverter.mpptInputs && "number of MPPT inputs", !inverter.maxPvVoltage && "maximum PV voltage", !inverter.mpptMin && "MPPT minimum", !inverter.mpptMax && "MPPT maximum", !inverter.maxInputCurrent && "maximum PV input current", batteries.length > 0 && !inverter.batteryVoltageMin && "battery voltage minimum", batteries.length > 0 && !inverter.batteryVoltageMax && "battery voltage maximum"].filter(Boolean);
...
function discoveryRequiredIssues(input: z.infer<typeof schema>) {
  const issues: string[] = [];
  for (const component of input.components) {
    const missing = [component.type === "inverter" && !component.manufacturer && "manufacturer", component.type === "inverter" && !component.model && "model"];
    ...
    if (component.type === "battery") missing.push(!component.batteryKwh && "capacity in kWh", !component.batteryAh && "capacity in Ah", !component.voltage && "nominal battery voltage", !component.batteryType && "battery type / chemistry");
    ...
    if (fields.length) issues.push(`${component.name}: enter ${fields.join(", ")}.`);
  }
  return issues;
}
...
export async function POST(request: Request) {
  ...
  const requiredIssues = parsed.data.discoveryContext?.continueDiscovery ? discoveryRequiredIssues(parsed.data) : [];
  if (requiredIssues.length) return Response.json({ error: "Complete the required equipment details.", blockers: requiredIssues }, { status: 422 });
  ...
  try {
    ...
    const project = await supabase.from("projects").update({ phase: parsed.data.discoveryContext?.continueDiscovery ? "discover" : "design", settings: {
      ...
      systemStatus: fromDiscovery ? "discovery" : "proposed",
