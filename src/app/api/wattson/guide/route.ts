...
const guideSchema = z.object({
  ...
  types: z.array(z.object({ name: z.string().max(180), description: z.string().max(1200), bestFor: z.string().max(800).optional(), watchFor: z.string().max(800).optional() })).max(30).optional(),
...
const requestSchema = z.object({
  ...
  guide: guideSchema,
  guideIndex: z.array(z.object({ id: z.string().max(120), title: z.string().max(240), group: z.string().max(160), aliases: z.array(z.string().max(120)).max(30).optional() })).max(250),
  recentConversation: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string().max(3000) })).max(8).default([]),
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return Response.json({ error: "Invalid guide question" }, { status: 400 });
