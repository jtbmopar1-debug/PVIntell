...
export default async function NewProposalPage({ searchParams }: { searchParams: Promise<{ site?: string; system?: string; from?: string; draft?: string; name?: string }> }) {
  ...
  return <ProposalIntake sites={sites.data ?? []} initialSiteId={query.site} initialSystemName={query.name} initialProposal={initialProposal} discoveryContext={query.from === "discovery" ? { draftId: query.draft } : undefined}/>;
}
