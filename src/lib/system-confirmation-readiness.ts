...
export async function systemConfirmationReadiness(supabase: SupabaseClient, projectId: string) {
  const [components, arrays, connections] = await Promise.all([
    supabase.from("system_components").select("id,display_name,confidence").eq("project_id", projectId),
    supabase.from("pv_arrays").select("id,name,confidence").eq("project_id", projectId),
  ...
  const componentBlockers = (components.data ?? []).filter((item) => item.confidence !== "confirmed").map((item) => item.display_name);
  const arrayBlockers = (arrays.data ?? []).filter((item) => item.confidence !== "confirmed").map((item) => item.name);
  const connectionBlockers = (connections.data ?? []).filter((item) => item.confidence !== "confirmed" || /compatibility warning:/i.test(item.notes ?? "")).map((item) => item.name);
