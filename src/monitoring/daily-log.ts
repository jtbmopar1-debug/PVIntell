...
const power = z.number().finite().positive().max(1000000).nullable();
export const dailyObservationSchema = z.object({
  ...
  // Optional because older daily records predate generator charging power.
  generatorChargingWatts: power.default(null),
  ...
  if (entry.generatorRan !== true && [entry.generatorMinutes, entry.generatorChargingWatts, entry.generatorStartSoc, entry.generatorEndSoc].some((value) => value !== null))
...
export const emptyObservation: DailyObservation = { actualKwh: null, gridImportKwh: null, gridExportKwh: null, socMin: null, socMax: null, socEnd: null, generatorRan: null, generatorMinutes: null, generatorChargingWatts: null, generatorStartSoc: null, generatorEndSoc: null, notes: "" };
...
export function dailyLogSummary(entries: DailyLogEntry[]) {
  ...
  const generatorChargeEstimates = entries.filter((row) => row.generatorRan === true && row.generatorMinutes !== null && row.generatorChargingWatts !== null);
